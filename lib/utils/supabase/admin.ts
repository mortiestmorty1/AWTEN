...
const upsertCustomerToSupabase = async (uuid: string, customerId: string) => {
  console.log('🔍 Debug: Attempting to upsert customer with admin client');
  console.log('🔍 Debug: UUID:', uuid, 'Customer ID:', customerId);

  // Test the role first
  const { data: roleTest, error: roleError } =
    await supabaseAdmin.rpc('auth.role');
  console.log('🔍 Debug: Role test result:', roleTest, roleError);
  ...
  console.log('✅ Debug: Customer upsert successful');
  return customerId;
};
...
/**
 * Process credit purchase from Stripe checkout
 */
const processCreditPurchase = async (
  ...
  try {
    // Use the fn_award_credits database function to add credits
    const { error: creditError } = await supabaseAdmin.rpc('fn_award_credits', {
      p_user_id: userId,
      p_amount: credits,
...
/**
 * Process premium subscription upgrade
 */
const processPremiumUpgrade = async (
  ...
  try {
    ...
    if (roleError) {
      ...
      throw new Error(
        `Failed to upgrade user to premium: ${roleError.message}`
      );
    }
    ...
    if (creditError) {
      ...
      console.warn(`Failed to award premium credits: ${creditError.message}`);
